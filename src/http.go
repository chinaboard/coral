...
type Request struct {
	...
	// request line from client starts at 0, coral generates request line that
...
func (r *Request) Verbose() []byte {
	...
	if r.isConnect {
		...
		// This includes client request line if has http upstream proxy
		rqbyte = r.raw.Bytes()
	}
	return rqbyte
}
...
// Used by both "Connection" and "Proxy-Connection" header. coral always adds
...
//
// Even though coral removes TE header, the original server can still respond
// with Trailer header.
...
// For now, coral does not fully support 100-continue. It will return "417
// expectation failed" if a request contains expect header. This is one of the
// strategies supported by polipo, which is easiest to implement in coral.
...
func parseRequest(c *clientConn, r *Request) (err error) {
	...
	// default, and I don't want to let others know the user is using coral, so
	// don't add it.
...
func parseResponse(sv *serverConn, r *Request, rp *Response) (err error) {
	...
	// Whether coral should respond with keep-alive depends on client request,
	// not server response.
	if r.ConnectionKeepAlive {
