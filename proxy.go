...
// http://www.mnot.net/blog/2011/07/11/what_servers_must_do
...
func (cp *coralProxy) toString() string {
	...
	return fmt.Sprintf("listen = coral://%s:%s@%s", method, cp.passwd, cp.addr)
}

func (cp *coralProxy) Addr() string {
	return cp.addr
}

func (cp *coralProxy) Serve(wg *sync.WaitGroup) {
	defer func() {
		wg.Done()
	}()
	ln, err := net.Listen("tcp", cp.addr)
	if err != nil {
		fmt.Println("listen coral failed:", err)
		return
	}
	info.Printf("coral proxy address %s\n", cp.addr)

	for {
		conn, err := ln.Accept()
		if err != nil {
			errl.Printf("coral proxy(%s) accept %v\n", ln.Addr(), err)
			if isErrTooManyOpenFd(err) {
				connPool.CloseAll()
			}
			time.Sleep(time.Millisecond)
...
func (c *clientConn) setReadTimeout(msg string) {
	// Always keep connections alive for coral conn from client for more reuse.
...
func isSelfRequest(r *Request) bool {
	...
	// But if client PAC setting is using coral server's DNS name, we can't
	// decide if the request is for coral itself (need reverse lookup).
...
func (c *clientConn) serveSelfURL(r *Request) (err error) {
	...
	sendErrorPage(c, "404 not found", "Page not found",
		genErrMsg(r, nil, "Serving request to coral proxy."))
	errl.Printf("cli(%s) page not found, serving request to coral %s\n%s",
...
func (c *clientConn) serve() {
	...
	var authed bool
	// For coral proxy server, authentication is done by matching password.
	if _, ok := c.proxy.(*coralProxy); ok {
		authed = true
	}
	...
	for {
		...
		// PAC may leak frequently visited sites information. But if coral
...
func (c *clientConn) connect(r *Request, direct bool) (srvconn net.Conn, err error) {
	...
	if upstreamProxy.empty() {
		errMsg = genErrMsg(r, nil, "No upstream proxy.")
		goto fail
	}
	...
	errMsg = genErrMsg(r, nil, "Upstream proxy connection failed.")
...
func (sv *serverConn) mayBeClosed() bool {
	if _, ok := sv.Conn.(coralConn); ok {
		debug.Println("coral upstream would keep alive")
		return false
	}
...
func (sw *serverWriter) Write(p []byte) (int, error) {
	if sw.rq.raw == nil {
		...
		// not buffered completely, coral can't retry and can release memory
		// immediately.
...
func (sv *serverConn) doConnect(r *Request, c *clientConn) (err error) {
	...
	if isHttpConn || isHttpsConn || isCoralConn {
		...
		if err = sv.sendHTTPProxyRequestHeader(r, c); err != nil {
			debug.Printf("cli(%s) error send CONNECT request to upstream: %v\n",
				c.RemoteAddr(), err)
			return err
		}
	} else {
...
func (sv *serverConn) sendHTTPProxyRequestHeader(r *Request, c *clientConn) (err error) {
	if _, err = sv.Write(r.proxyRequestLine()); err != nil {
		return c.handleServerWriteError(r, sv, err,
			"send proxy request line to http upstream")
	}
	if hc, ok := sv.Conn.(httpConn); ok && hc.upstream.authHeader != nil {
		// Add authorization header for upstream http proxy
		if _, err = sv.Write(hc.upstream.authHeader); err != nil {
			return c.handleServerWriteError(r, sv, err,
				"send proxy authorization header to http upstream")
		}
	} else if hc, ok := sv.Conn.(httpsConn); ok && hc.upstream.authHeader != nil {
		// Add authorization header for upstream http proxy
		if _, err = sv.Write(hc.upstream.authHeader); err != nil {
			return c.handleServerWriteError(r, sv, err,
				"send proxy authorization header to https upstream")
		}
	}
	// When retry, body is in raw buffer.
	if _, err = sv.Write(r.rawHeaderBody()); err != nil {
		return c.handleServerWriteError(r, sv, err,
			"send proxy request header to http upstream")
	}
	/*
...
func (sv *serverConn) sendRequestHeader(r *Request, c *clientConn) (err error) {
	...
	switch sv.Conn.(type) {
	case httpConn, httpsConn, coralConn:
...
func sendBody(w io.Writer, bufRd *bufio.Reader, contLen int, chunk bool) (err error) {
	...
	// coral does not sanitize response header, but can correctly handle it
	if chunk {
